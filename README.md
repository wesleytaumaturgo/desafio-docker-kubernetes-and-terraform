...
- **Senha gerada pelo Terraform.** `random_password` (24 caracteres alfanuméricos, porque ela entra na `DATABASE_URL`) é entregue ao chart por `set_sensitive`. O `values.yaml` versionado tem `db.password: ""`, e o template usa `required`: renderizar sem senha falha. No cluster, a credencial existe no Secret `mural-db` e no Secret de release do Helm (`sh.helm.release.v1.mural.v1`, que guarda os values do `set_sensitive`); nos Deployments, ConfigMaps e Job ela só entra por `secretKeyRef`. Fora do cluster, existe no state local do Terraform (ver [Limitações](#limitações)).
...
- **Onde a senha existe.** No cluster: Secret `mural-db` e Secret de release do Helm. No disco: `terraform.tfstate` enquanto o cluster existe; `terraform.tfstate.backup` depois do `destroy`. Em nenhum arquivo versionado.
